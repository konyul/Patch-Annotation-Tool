...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.corruption_dock = QtWidgets.QDockWidget(self.tr("Corruption"), self)
        self.corruption_dock.setObjectName("Corruption")

        corruption_layout = QtWidgets.QVBoxLayout()
        #위쪽으로 딱 붙게
        corruption_layout.setContentsMargins(0, 0, 0, 0)
        corruption_layout.setSpacing(5)
        ...
        for class_id in ["class1", "class2", "class3", "class4", "class5", "class6"]:
            ...
            palette.setColor(QtGui.QPalette.Window, QtGui.QColor(192, 192, 192))  # Default to gray
            label.setAutoFillBackground(True)
            label.setPalette(palette)
            corruption_layout.addWidget(label)
            self.class_labels.append(label)

        intensity_label = QtWidgets.QLabel("Select Intensity:")
        corruption_layout.addWidget(intensity_label)
        self.intensity_combo = QtWidgets.QComboBox()
        self.intensity_combo.addItems(["BLURRY", "BLOCKAGE"])
        corruption_layout.addWidget(self.intensity_combo)
        ...
        for intensity in ["BLURRY", "BLOCKAGE"]:
            label = QtWidgets.QLabel(intensity)
            label.setAlignment(QtCore.Qt.AlignCenter)
            label.setFixedHeight(30)
            palette = label.palette()
            palette.setColor(QtGui.QPalette.Window, QtGui.QColor(192, 192, 192, self.intensity_color_map[intensity]))  # Default to gray with correct alpha
            label.setAutoFillBackground(True)
            label.setPalette(palette)
            corruption_layout.addWidget(label)
        ...
        createPatchMode = action(
            self.tr("Create Patch"),
            lambda: self.toggleDrawMode(False, createMode="patch_annotation"),
            shortcuts["create_patch"], 
            "objects",
            self.tr("Start drawing points"),
            enabled=False,
        )
        ...
        self.actions = utils.struct(
            ...
            createPatchMode=createPatchMode,
        ...
        utils.addActions(
            ...
            (
                ...
                self.corruption_dock.toggleViewAction(),
                None,
                fill_drawing,
                None,
                hideAll,
    ...
    def updateClassAndIntensity(self, class_text, intensity_text):
        if class_text:
            index = self.class_combo.findText(class_text, QtCore.Qt.MatchFixedString)
            if index >= 0:
    ...
    def updateSelectionColor(self):
        ...
        selected_intensity = self.intensity_combo.currentText()
        ...
        for label in self.class_labels:
            ...
            if class_id == selected_class:
                color = self.class_color_map[class_id]
                if selected_class == "CLEAN":
                    color = QtGui.QColor(192, 192, 192)  # Gray color for CLEAN
                else:
                    color.setAlpha(self.intensity_color_map.get(selected_intensity, 255))
                palette.setColor(QtGui.QPalette.Window, color)
            else:
                palette.setColor(QtGui.QPalette.Window, QtGui.QColor(192, 192, 192))
            label.setPalette(palette)

        # Update intensity labels
        for label in self.intensity_labels:
            ...
            if intensity == selected_intensity:
                if selected_class == "CLEAN":
                    ...
                    color = self.class_color_map[selected_class]
                    color.setAlpha(self.intensity_color_map.get(intensity, 255))
                palette.setColor(QtGui.QPalette.Window, color)
            else:
                gray = QtGui.QColor(192, 192, 192)
                gray.setAlpha(self.intensity_color_map.get(intensity, 255))
                palette.setColor(QtGui.QPalette.Window, gray)
            label.setPalette(palette)
    ...
    def setClean(self):
        ...
        self.actions.createPatchMode.setEnabled(True)
    ...
    def toggleDrawMode(self, edit=True, createMode="polygon"):
        draw_actions = {
            ...
            "patch_annotation": self.actions.createPatchMode,
    ...
    def newShape(self, patch_label=None):
        ...
        if patch_label is not None:
            #print("class :",self.class_combo.currentText())
            #print("corruption :",self.intensity_combo.currentText())
            ...
            class_label = class_map.get(class_text, "")
